...
@section('content')
    ...
    <x-page-header
        ...
        subtitle="Kelola semua kursus yang tersedia di platform"
    >
        <x-slot:actions>
            <button onclick="EP.toast.info('Filter belum tersedia')"
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div class="ep-card stat-card">
            <div class="ep-card-body flex items-center gap-3">
                ...
                <div>
                    <p class="text-xl font-display font-bold text-slate-800 dark:text-white leading-tight">{{ $stats['total'] }}</p>
        ...
        <div class="ep-card stat-card">
            <div class="ep-card-body flex items-center gap-3">
                <span class="text-2xl">✅</span>
                <div>
                    <p class="text-xl font-display font-bold text-slate-800 dark:text-white leading-tight">{{ $stats['published'] }}</p>
        ...
        <div class="ep-card stat-card">
            <div class="ep-card-body flex items-center gap-3">
                <span class="text-2xl">⏳</span>
                <div>
                    <p class="text-xl font-display font-bold text-slate-800 dark:text-white leading-tight">{{ $stats['draft'] }}</p>
        ...
        <div class="ep-card stat-card">
            <div class="ep-card-body flex items-center gap-3">
                <span class="text-2xl">📦</span>
                <div>
                    <p class="text-xl font-display font-bold text-slate-800 dark:text-white leading-tight">{{ $stats['archived'] }}</p>
    ...
    <x-table
        :columns="[
            ...
            ['key' => 'students',   'label' => 'Siswa',        'sortable' => true],
    ...
    >
            ...
            <tr>
                ...
                <td>
                    <div class="flex items-center gap-3">
                        <div class="w-9 h-9 rounded-xl bg-brand-50 dark:bg-brand-500/10 flex items-center justify-center text-base flex-shrink-0">
                            📚
                        </div>
                        <div>
                ...
                <td>
                    <span class="font-semibold text-slate-700 dark:text-slate-200">
                        {{ $course->chapters->sum(fn($ch) => $ch->lessons->count()) }}
                    </span>
                </td>
                ...
                <td>
                    <div class="flex items-center gap-1">
                        ...
                        <a href="{{ route('admin.courses.show', $course->id) }}"
                           class="ep-btn-icon ep-btn-secondary ep-btn-sm"
                           title="Lihat">
                        ...
                        <a href="{{ route('admin.courses.edit', $course->id) }}"
                           class="ep-btn-icon ep-btn-outline ep-btn-sm"
                           title="Edit">
                            <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <button
                            ...
                            class="ep-btn-icon ep-btn-danger ep-btn-sm"
                            title="Hapus">
                            <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <x-slot:footer>
            <div class="flex items-center justify-between text-sm text-slate-500">
                <span>Menampilkan {{ $courses->firstItem() ?? 0 }}–{{ $courses->lastItem() ?? 0 }} dari {{ $courses->total() }} kursus</span>
                <div class="flex gap-1">
                    @if ($courses->onFirstPage())
                        <span class="px-3 py-1 rounded-lg bg-slate-100 dark:bg-navy-700 text-slate-400 cursor-not-allowed">←</span>
                    @else
                        <a href="{{ $courses->previousPageUrl() }}" class="px-3 py-1 rounded-lg bg-slate-100 dark:bg-navy-700 hover:bg-brand-50 dark:hover:bg-brand-500/10 text-slate-700 dark:text-slate-300">←</a>
                    @endif
                    ...
                    @foreach ($courses->getUrlRange(1, $courses->lastPage()) as $page => $url)
                        ...
                        @else
                            <a href="{{ $url }}" class="px-3 py-1 rounded-lg bg-slate-100 dark:bg-navy-700 hover:bg-brand-50 dark:hover:bg-brand-500/10 text-slate-700 dark:text-slate-300">{{ $page }}</a>
                        @endif
                    @endforeach
                    
                    @if ($courses->hasMorePages())
                        <a href="{{ $courses->nextPageUrl() }}" class="px-3 py-1 rounded-lg bg-slate-100 dark:bg-navy-700 hover:bg-brand-50 dark:hover:bg-brand-500/10 text-slate-700 dark:text-slate-300">→</a>
                    @else
                        <span class="px-3 py-1 rounded-lg bg-slate-100 dark:bg-navy-700 text-slate-400 cursor-not-allowed">→</span>
                    @endif
                </div>
