...
<aside class="sidebar sidebar-bg z-30 flex-shrink-0 flex flex-col"
    ...
    {{-- Logo --}}
    <div class="flex items-center px-5 py-5 border-b border-white/5"
        ...
        <div class="flex items-center gap-3">
            ...
            <span x-show="!sidebarCollapsed" x-cloak class="font-display font-bold text-lg text-white tracking-tight">
                EduPanel
            </span>
        </div>
        <button x-show="!sidebarCollapsed" x-cloak @click="sidebarCollapsed = true"
            class="text-slate-400 hover:text-white p-1 rounded-lg hover:bg-white/5 transition-colors hidden lg:block"
            title="Tutup sidebar">
    ...
    <button x-show="sidebarCollapsed" x-cloak @click="sidebarCollapsed = false"
        class="mx-auto mt-3 text-slate-400 hover:text-white p-2 rounded-lg hover:bg-white/5 transition-colors hidden lg:flex"
        title="Buka sidebar">
        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <nav class="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
        ...
        <p x-show="!sidebarCollapsed" x-cloak
            class="text-xs font-semibold text-slate-100 uppercase tracking-widest px-3 mb-2 mt-1">
            Menu Utama
        </p>
        ...
        @foreach ($navItems as $item)
            <a href="{{ $item['href'] }}"
                ...
                :class="sidebarCollapsed ? 'justify-center' : ''" @class([
                    'nav-item-active text-brand-400' => $activeNav === $item['key'],
        ...
        <p x-show="!sidebarCollapsed" x-cloak
            class="text-xs font-semibold text-slate-500 uppercase tracking-widest px-3 mb-2">
            Pengaturan
        </p>
    ...
    <div class="p-3 border-t border-white/5">
        <div class="flex items-center gap-3 p-2 rounded-xl hover:bg-white/5 transition-colors"
            ...
            <form x-show="!sidebarCollapsed" x-cloak action="{{ route('logout') }}" method="POST"
                class="flex-shrink-0">
                @csrf
                <button type="submit" class="text-slate-500 hover:text-slate-300 transition-colors p-1" title="Logout">
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
